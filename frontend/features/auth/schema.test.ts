...
describe("auth Zod 스키마", () => {
  ...
  test("signupSchema: 약한 비밀번호(숫자 없음) 실패", () => {
    const r = signupSchema.safeParse({
      email: "a@b.com", password: "abcdefgh", passwordConfirm: "abcdefgh", name: "홍길동",
    });
    expect(r.success).toBe(false);
  });
  test("signupSchema: 비밀번호 불일치 실패", () => {
    const r = signupSchema.safeParse({
      email: "a@b.com", password: "abcd1234", passwordConfirm: "abcd9999", name: "홍길동",
    });
    expect(r.success).toBe(false);
  });
  test("signupSchema: 올바른 입력 통과", () => {
    const r = signupSchema.safeParse({
      email: "a@b.com", password: "abcd1234", passwordConfirm: "abcd1234", name: "홍길동",
    });
