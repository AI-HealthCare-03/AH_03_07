...
import { ArrowRight, Check, Circle, AlertTriangle, ClipboardCheck, FileText, Stethoscope } from "lucide-react";
            ...
            {records.map((r) => (
              <div key={r.id} className="flex items-center gap-3 px-4 py-3.5">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-sky-50">
                  <Stethoscope className="h-5 w-5 text-sky-500" strokeWidth={1.5} />
                </div>
                <div className="flex-1">
                ...
                <span className="ml-2 shrink-0 text-xs text-muted-foreground">{shortDate(r.visit_date)}</span>
              </div>
            ))}
